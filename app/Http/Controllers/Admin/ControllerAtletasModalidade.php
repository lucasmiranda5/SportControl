...
class ControllerAtletasModalidade extends Controller
{
    ...
    public function formulario(){
		...
		$eventos = Eventos::all();
		return view('admin.equipe.formulario')->with('acao','novo')->with('instituicoes',$instituicoes)->with('msg',$msg)->with("eventos",$eventos);		
	}	
	function campus($id){
	...
	function modalidades(){
		$campos = Request::all();
		$mod = ModalidadeCampus::where("evento",$campos['evento'])->where("campus",$campos['campus'])->get();
		$arr = [];
		$x = 0;
		foreach($mod as $m){
	...
	function chamarAtletas(){
		...
		if($retorno['sub']== $retorno['id'] or $retorno['sub'] == ''){
			...
			$retorno2 = AtletasModalidade::where('modalidade',$retorno['sub'])->where("evento",$campos['evento'])->where("campus",$campos['campus'])->get();
		...
		$retorno3 =AtletasModalidade::where("modalidade",$campos['modalidade'])->where('campus',$campos['campus'])->where('evento',$campos['evento'])->get();
		?>
